...
public class LotteryServiceIpml implements LotteryService {
    ...
    public ResponseResult txAttentionLottery(concernPo concernPo) {
        ...
        concernPo concern = concernPoMapper.selectEntityByMap(dto);
        if(ObjectUtils.isEmpty(concern)){
        ...
        concern.setIsConcern(Constant.IsConcern.source_isconcern_yes);
        concern.setUpdateDate(new Date());
        return ResponseResult.success(concernPoMapper.updateByPrimaryKeySelective(concern));
    }
